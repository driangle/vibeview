...
- [ ] Add an `apps/lib/timeline` package with an `Exchange` struct and a builder,
      e.g. `BuildExchanges(messages []claude.Message) []Exchange`.
- [ ] Per exchange, compute: `index`, `startTime`/`endTime`/`durationMs` (from message
...
- [ ] Match the existing badge/flag semantics (deepThinking, approvalGate = last block of
      last assistant message is `tool_use`, etc.) so it matches the current view.
- [ ] Table-driven tests over representative message sequences: multi-tool loop exchange,
