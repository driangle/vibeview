...
3. Read `apps/cli/internal/server/*.go` to identify API endpoints served by the backend.
4. Compile a list of:
...
1. Read `apps/cli/cmd/vibeview/main.go` and `apps/cli/internal/settings/` to extract global flags and config loading.
2. Read any config-related files (grep for config file references or `viper` usage) to find all supported config keys.
3. Check for any example config files.
...
1. **`apps/docs/guide/cli.md`** (docs site):
...
1. **`apps/docs/guide/web-interface.md`** (docs site):
...
1. Check `apps/docs/getting-started/installation.md` and `apps/docs/getting-started/quick-start.md`:
...
1. For each gap identified in Phase 6, update the relevant documentation file:
   ...
   - Add missing web feature descriptions to `apps/docs/guide/web-interface.md`
